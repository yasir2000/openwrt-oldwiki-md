...
== How? ==
I accidently uploaded the firmware for a [http://wiki.openwrt.org/hardware/belkin/F5D7230 F5D7230] to my [http://wiki.openwrt.org/hardware/belkin/F5D7130 F5D7130] and it worked just fine ;-) So I went ahead an uploaded OpenWRT RC5. It works a treat!

=== Installation method ===
I'm not sure if you need to flash the router to be a [http://wiki.openwrt.org/hardware/belkin/F5D7230 F5D7230] first of all or not, but I did. I got the firmware for that from [http://networking.belkin.com/update/files/uk/54g_router.html here] (this links to the embeded firmware update page for the UK).
 ...
 * Once you've flashed the [http://wiki.openwrt.org/hardware/belkin/F5D7130 F5D7130] to be an [http://wiki.openwrt.org/hardware/belkin/F5D7230 F5D7230] you'll notice on the default status screen (index.html) that it says NAT disabled. If you click the word NAT it links to the admin password page which also has the NAT setting saying disabled. If you enable this you'll unlock all the firewall etc features of the [http://wiki.openwrt.org/hardware/belkin/F5D7230 F5D7230].
