...
BREAKTHROUGH! I found the API for programming the crypto API of the bcm5365P or Broadcom 5365P. Unfortunately my skills are limited with programming DMAs and such, so you can get the file [http://voodoowarez.com/bcm5365p.pdf here]. The crypto api description begins on page 60. It includes everything required to build a driver as it seems that no driver can be found so far (except as part of the Broadcom Dev environment and no one seems to be able to get that).
