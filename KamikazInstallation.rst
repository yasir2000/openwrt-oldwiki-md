...
There are several ways to install Kamikaze, but this will only cover the very basic procedure based on using the firmware upgrade function of the web interface (web^if). 
This means in praxis, that all which is needed to be done is to download the precompiled Kamikaze image and then simply upgrade the device.
Kamikaze supports a rather large number of different platforms (https://dev.openwrt.org/wiki/platforms) so it is required to download the correct image from http://downloads.openwrt.org/kamikaze/7.09/ 
In the case of the WRT54G the correct image resides in the brcm-2.4/ directory - in this directory the correct file to download would have the name: openwrt-wrt54g-2.4-squashfs.bin
...
1. There is no build-in web interface.
