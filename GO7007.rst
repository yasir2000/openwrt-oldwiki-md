...
'''Hint:''' Currently the driver loads and at least spook is working, but only with mjpeg and not mpeg4. Accessing the audio devices will cause a kernel crash and you have to reboot then.
...
MPEG4 does not work without many disturbance in the video. However MJPEG works good but leads to an average bandwidth of 4MBit/s. MPEG4 generates approximatly 1MBit/s. The CPU load is quite low with about 10% (same to memory), delay is not noticable even if using mpeg4.
==== wis-streamer ====
