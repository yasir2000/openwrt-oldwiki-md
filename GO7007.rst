...
==== Spook ====
'''Hint: '''__''The video can only be watched with mplayer, VLC and Quicktime fail due to bad/large/corrputed packets.''__

First clean up your source by calling "make distclean" in order to rebuild the toolchain. The change to uClibc requires this step. Then edit the file "toolchain/uClibc/patches/120-more_standard_math.patch" and add the functions "s_floorf.c s_ceilf.c"[http://openwrt.pbwiki.com/Adding_Math_Functions_To_uClibc_mathlib http://openwrt.pbwiki.com/Adding_Math_Functions_To_uClibc_mathlib]
