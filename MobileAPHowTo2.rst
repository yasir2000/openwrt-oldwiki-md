...
(thanks Kaloz), my first thought was "what an awkward product-name!". I just call it SL to keep it short.
After that I turned to the many uses for the USB port. One obvious one was using a cellular phone to attach to
...
= Caveats =
 ...
 4. Would be nice to have a local clock source, as the SL has no clock and comes up with wrong time at boot. PPP does note the large time disparity in the logs. You can sync after connecting with rdate or ntpclient.
