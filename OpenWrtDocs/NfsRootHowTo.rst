...
The OpenWrt-specific parts are [wiki:Self:OpenWrtDocs/BuildingKamikazeHowTo building a custom image] and changing the kernel command line.  Changing the kernel command line varies from device to device, but it generally involves either a serial, telnet, or tftp connection into the bootloader.  See the page for the device on the TableOfHardware for more information.
