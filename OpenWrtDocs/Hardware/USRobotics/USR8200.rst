...
=== Opening Up the Clamshell ===
Under the Mac Address label on the bottom side is a hidden screw. Here's a picture.[[BR]]
[[ImageLink(http://home.comcast.net/~johnrw/image/hiddenScrew-1.jpg, target[width=1024,height=768])]][[BR]]
Clear? Remove this screw before messing with the 7 tabs. Once the screw is out, remove the 4 rubber pads on the feet. I usually start on the top right corner with a small technician's flat screwdriver. It helps to have a second person. The second person uses a hard plastic edged thing you may have laying around... and pries upward in the groove that runs all around the housing, on the side. Good luck. It seems that everyone breaks the first tab on thier first time... so if you do, it's really okay. There are 6 more to hold it all in place when you are all finished.

=== USR8200 GPIO assignments ===
|| GPIO[0] || USB-1 LED || OUTPUT ||
|| GPIO[1] || USB-2 LED  ||  OUTPUT ||
|| GPIO[2] || IEEE1394 LED || OUTPUT ||
|| GPIO[3] || Wireless LED * || OUTPUT ||
...
|| GPIO[14] || Power LED  || OUTPUT ||
|| GPIO[15] || PCI Clock || INPUT ||

* The wireless LED GPIO[3] is not visible from the outside... as the cutout was never cut out because wireless was never added in a RTM model.
...
Setting the GPCLCKR register's MUX14 bit to zero enable GPIO control of GPIO[14], and thus allowing us to turn the Power LED to GREEN.
On power on, the cpu sets this to zero... which made it a little elusive.
[[BR]]mfill -b 0xc8004018 -l 4 -p 0x01100000 -4
[[BR]]We will add these to the port... so you won't have to. :)
...
JW: Update: A 4th wire, chassis ground was added to the homemade serial cable. Soldered to the D-sub housing on one end and an alligator clip on the other. Clipping the alligator clip to the USB shield housing was a good enough location. Reset bug solved. When using the Olimex, I never experienced it, but had some time to add the 4th wire to my spare today.
...
Here is the pcb top side.

[[ImageLink(http://home.comcast.net/~johnrw/image/M5110012.JPG, target[width=1024,height=768])]]

== USB Wireless w/Access Point Mode Added ==

On Jan 3, 2009 John reached a project goal of his. AP mode with WPA-PSK encryption wireless support. Using the OpenWRT buildroot, and the compat-wireless branch from Dec 29, 2008, and Linux 2.6.28 he added support for the Linksys WUSB54GV2 adapter through the p54 driver. He reported the kernel had no trouble setting the card to Master mode using hostapd. He pinged the USR8200/WRT54GV2 all day with no significant packet loss. [[BR]]
The Linksys USB adapter has a ISL3887(prism54 softmac) chip, which needed p54common, p54pci, p54usb and the new mac80211 Linux wireless stack. Well that turned out to be a great New Year's Day present from linux-wireless!
...
Since I had to do a lot of experimenting, I have tons of these files. I expect to make changes to these... but here... for now.
This uses OpenOCD rev 728. I suggest you get that revision if you want to use these files. It works well enough with the cpu on this unit to even single step/set breakpoints in linux code.  These config files will not help you do that. They are posted to allow OpenOCD to write an image to the flash chip.
...
# [trst-type] [srst-type] ? signals is one of none, trst only, srst only or trst and srst.
# combination is one of srst pulls trst, trst pulls srst,combined, separate.
...
###############################################################################
# target script <target#> <event> <scriptfile> event is either reset, post halt or pre resume.
...
flash write_image erase /var/www/redboot.bin 0x50000000 bin
...
== Using the collectd-exec plugin
Recently, as part of a different project, I had need to use the collectd-exec plugin. It needs it's own wiki page, as it has a couple gotcha's.

 1. It refuses to execute anything as user root. Upon examing the source  http://www.sfr-fresh.com/unix/privat/collectd-4.4.5.tar.gz:a/collectd-4.4.5/src/exec.c beginning at line 290:{{{
  290   uid = sp.pw_uid;
  291   gid = sp.pw_gid;
  292   if (uid == 0)
  293   {
 ...
 Solution: I discovered it would allow execution of programs as uid:nobody,gid:root.

 1. The environment created in /etc/profile is not inherited. 
    Since I was trying to run the python interpreter, I needed to set some environment variables. I did not experiment to see what environment variables are there for example... but noticed that they were not set after I added them to /etc/profile. Stuff like this... essential for correct python operation... especially when it is installed in a nonstandard location. Mine is installed to /mnt/sda1/python, a removable flash drive, so these needed to be inherited from collectd's environment.
{{{
PATH=/mnt/sda1/python/usr/bin:$PATH
export PATH

PYTHONHOME=/mnt/sda1/python/usr/
export PYTHONHOME

PYTHONPATH=/mnt/sda1/python/usr/lib/python25.zip:\
/mnt/sda1/python/usr/lib/python2.5/:\
/mnt/sda1/python/usr/lib/python2.5/plat-linux2:\
/mnt/sda1/python/usr/lib/python2.5/lib-tk:\
/mnt/sda1/python/usr/lib/python2.5/lib-dynload:\
/mnt/sda1/python/usr/lib/python2.5/site-packages:\
/mnt/sda1/python/usr/lib/python2.5/sqlite3

export PYTHONPATH
}}}

Solution: Add them to /etc/init.d/collectd instead.
