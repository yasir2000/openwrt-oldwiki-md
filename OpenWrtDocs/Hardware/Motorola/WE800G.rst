...
Note: Verify if the boot_wait was indeed set or not with the "nvram get boot_wait" command.
 ...
 * Reboot the WE800G or you get wierd permission or other type of errors when you try to change the configuration of this WE800G;
...
Note: There is no WAN interface, use WIFI instead (you may need to set wifi_ipaddr, wifi_netmask, wifi_proto, wifi_gateway, ... [:OpenWrtNVRAM: variables]).
...
Note2: There is no WAN interface, only LAN is in use. You may need to set lan_ipaddr, lan_netmask, lan_proto, lan_gateway, ... [:OpenWrtNVRAM: variables].
...
Note: There is no WAN interface, only LAN is in use. You may need to set lan_ipaddr, lan_netmask, lan_proto, lan_gateway, ... [:OpenWrtNVRAM: variables].
