...
[[TableOfContents]]
= Hardware versions =
There are three different WE800G hardware versions - two types of v1 (with differing memory sizes) and v2.

== WE800G v1 ==
The WE800G v1 is based on the Broadcom 4710 reference design. It has a 125MHz BCM4702 CPU, and the radio is based on a BCM4306 on a mini-PCI card. The early production units were equipped with 4MB flash and 16MB SDRAM, but this was reduced to 2MB and 8MB in the later production - it appears to be impossible to tell which variant you have from outside the box. 

There is a connector that has the required signals for connecting a serial port (you need an external UART and line driver), but no JTAG connector (the signals are there, but on tiny SMT resistors).
...
goodnewscd added: I have the version 2.   I cannot see any different between v1 and v2 from outside.  It also has the same mini-PCI card; however, it only has 2Mb flash and 8Mb SDRAM. It is not that useful even using the micro version of OpenWRT. I have to build my only "super-micro" version.
...
This needs to be clarified - I have two of these units - one has a 4/16MB configuration (shipped with V2.0 firmware) and the other is 2/8MB (shipped with 2.09 firmware).

Apart from this, the hardware is identical - even down to the PCB part numbers.  The 4/16 unit has an AMD AM29LV320DB-90 flash and a pair of AMIC A43L2616V-6 1Mx16x4 SDRAMS.  The 2/8 unit has a Fujitsu 29LV160BE-70 flash and a pair of Dynachips D98SD6416AH-6 (1Mx16x2?) SDRAMS.

The PCB has the J25 header that contains the pins needed for a serial port, but does not appear to have any provision for JTAG. 
