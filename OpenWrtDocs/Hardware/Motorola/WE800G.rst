...
Web based upgrade from pre-RC5 to RC5 failed with this message:
{{{
Firmware Upgrade
...
And failed even after making the proposed .bin to .trx conversation.

Only way to upgrade the firmware is the TFTP based upgrade as was described above.

Note: The WEB based firmware upgrade does not work even after the upgrade to RC5.
