...
= Port layout =
The WE800G has 3 LAN ports and a bridge port. The eth0 has no physical connector and is dummy port, the eth1 is the LAN RJ45 port and the eth2 is wireless card port. The br0 is bridge port.

= Client Mode =
Out of box the WE800G is set to client mode, but it does not work - you can ping the box from both sides just fine, but it does not let any of these packet to travel over the defined bridge. 

== Routed Client Mode ==


== Bridged Client Mode ==
To put the WE800G into the bridged client mode you have to bridge LAN and WLAN port, disable separate wifi device and put the radio into WET mode.
...
Note: Somehow I didn't get this mode going before I installed and then removed the WL package. I wonder why?
