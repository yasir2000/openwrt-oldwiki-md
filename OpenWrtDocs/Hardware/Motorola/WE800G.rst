 ...
 * Upload the .trx image (Control Panel->Firmware Upgrade);
...
Note: After successful flashing the IP address of WE800G will be set to 192.168.1.1/24 and the box should start answering to pings on that address.

 * After successful flashing wait for some minutes to let the OpenWRT to finish all it's upgrade procedures;
