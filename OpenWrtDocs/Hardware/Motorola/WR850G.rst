...
The Broadcom wireless is not (yet?) supported by an open source driver.  The proprietary driver is only available for Linux kernel 2.4.  If you need to use wireless, you need to use a version of OpenWRT based on the 2.4 kernel, not the main version of Kamikaze which is based on the 2.6 kernel.
