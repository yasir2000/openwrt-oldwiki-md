...
/!\ Some users report that boot_wait only functions for one reboot after setting it from the web interface. To be sure that is the case reboot the WR850G after to "boot_wait=on" procedure and restart it. If the box is up again then issue "nvram get boot_wait" on the very same /frame_debug.asp page. To my testing the WR850G v2 does act like this.
...
== Alternate Initial Loading Method ==
The '''nvram set boot_wait=on''' method above did not put the router into 192.168.10.1 for me so I tried the following. I believe I had already upgraded my WR850g per:  [[BR]]
WR850G 4.03 Firmware Release Notes [[BR]]
Part Number 515750-001[[BR]]
Product WR850G[[BR]]
Release Version 4.03[[BR]]
Date 05/04[[BR]]
[[BR]]
With the version 4.03 the '''control''' page has an option '''upgrade firmware'''. I tried this and it complained that the openwrt-wr850g-squashfs.bin file was not a trx file. I renamed the file to .trx instead and it uploaded. After the upload the modem light blinked red for about a minute. [[BR]]
Initially I was unable to connect to the router however, I followed the Motorola instructions and unplugged the device and went and made tea. When I plugged it back in, I put into a different pc and using the original IP address (I had used 192.168.1.5) in the brower pulled up the openwrt welcome page. BINGO and a lot easier. [[BR]]

Note: When I first loaded OpenWrt, I was able to find the IP for the device. I ran etheral protocol analyzer and only saw spanning tree protocol messages from the router's ethernet mac address and about every fourth message was a ppoe query. I couldn't find the IP address of the router using etheral. [[BR]] 
...
Question: If this comment is true
...
Then does the WR850G need to keep LAN port 0, 1 or 3 in VLAN0?
