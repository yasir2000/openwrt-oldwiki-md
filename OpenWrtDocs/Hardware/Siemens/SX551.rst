...
The serial signals are at a 3.3V level, so you need to use a level convertor, see [:OpenWrtDocs/Customizing/Hardware/Serial_Console]
...
Things i've dscovered so far:
Pressing u or U gives a list of some (non-intresting) debug infou about transmitted and received packets.
