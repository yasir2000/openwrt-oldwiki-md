...
=== RAM Upgrade ===
__ /!\ __ Upgrading memory will void the warranty and it requires a steady hand and good soldering skills.
  
The SE505V2 has one 4Mx16 SDRAM chip (8 MB), and spare pads for a second RAM chip. RAM initialization in the CFE seems to be fixed to 4Mx16 (it gnores the sdram_init value in the nvram). 8Mx16 chips will be used as 4Mx16 chips. Mixing 4Mx16 and 8Mx16 is possible.  32MB are reported to work by using two 16Mx16 chips.

Instructions to upgrading from 8MB to 16MB:

1) find the right SDRAM chips:
64MB or 128MB SODIMMS are a good source for SDRAM chips with a bus width of 16 bit (always check the data sheet!). Unsoldering the chips with a hot-air gun works fine (be careful not to overheat the chips!). 
...
Hint: desoldering wick and a strong (50W) soldering iron works better than an expensive soldering needle! Do not overheat the pads and doubble check for short circuits between pins. 

5) connect the address lines for the second RAM chip:

{{{
shorten the pads with the following labels using really fine wire:
  R314
  R315
  R316
  R317
