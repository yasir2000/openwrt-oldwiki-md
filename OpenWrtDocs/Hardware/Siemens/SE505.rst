...
== RAM upgrade ==
The SE505V2 can be easily modded to 16 MB RAM, but it requires good soldering skills. The SE505V2 has one 4Mx16 SDRAM chip (8 MB), and spare pads for a second RAM chip. RAM initialization in the CFE seems to be fixed to 4Mx16 (it gnores the sdram_init value in the nvram). However, 32MB are reported to work, when using two 16Mx16 chips.
...
and solder a 4Mx16 sdram on the second RAM-spot. After booting the router you should have 16MB ! :)
...
Hint: don't solder a capacitor to C319! I soldered one and it didn't boot anymore... unsoldered it, and it worked.
...
On the software side: the SE505V2 has a OHCI controller.
