...
64MB to 256MB SODIMMS are a good source for SDRAM chips with a bus width of 16 bit (always check the data sheet!). Unsoldering the chips with a hot-air gun works fine (be careful not to overheat the chips!). 

2) solder a 4Mx16 SDRAM chip to the unpopulated RAM pads.

3) connect the address lines for the second RAM chip:
