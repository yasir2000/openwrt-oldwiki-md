...
== Stock Firmware ==
'''1.''' Connect to the router with a ethernet cable on LAN port 2, 3 or 4 (1 is not yet active). Tftp the new image to the router: Unplug router, replug it back in while holding in the black reset button. Wait for a slowly blinking power light before releasing the black reset button. The remainder of the TFTP process is described here: OpenWrtViaTftp.
