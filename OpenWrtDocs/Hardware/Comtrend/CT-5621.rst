...
If you want to play with the stock firmware and know the required offsets and size, you can use the [attachment:make-flash-image.tar.gz make-flash-image.tar.gz] to create imagetag, which will generate a correct image header so you can flash the modified firmware to the router (using cfe).
