...
CFE networking appears not to work, or is attached to a device other than the switch for the LAN ports, as it appears to be impossible to get the CFE bootloader to download anything from a tftp server and 'put'ting a file at the router/modem via tftp doesn't work either.
