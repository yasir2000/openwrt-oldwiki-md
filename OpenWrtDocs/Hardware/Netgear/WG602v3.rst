...
=== Kamikaze ===
As of revision 3603, Kamikaze appears to be basically working except for the eth0 module.
Because of it, /sbin/ifconfig hangs, and the boot process doesn't complete (eg., firstboot is never called, and the j2ffs partition never made active).
If you remove /sbin/ifconfig from your root image, Kamikaze will boot without issues through the kernel, init, to a working shell.

Actually, you may have to remove some stuff in ordeer to free RAM and Flash space, as this access point is very limited. Anyway, you can get:
 . {{{
CFE version 1.0.37.6 for BCM947XX (32bit,SP,LE)
