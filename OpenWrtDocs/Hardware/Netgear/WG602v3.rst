...
Console access to the AP can be enabled both on ttyS0 and ttyS1. The shell is started by `/sbin/init` after the kernel has booted, as configured in `/etc/inittab`. The file to modify for Kamikaze is `package/base-files/default/etc/inittab`. Change
{{{
