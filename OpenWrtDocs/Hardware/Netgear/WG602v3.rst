...
=== Serial console ===
This AP uses by default UART1 (/dev/ttyS1) as the console, while OpenWRT is usually configured to use UART0 (/dev/ttyS0). If you want to use the serial console on /dev/ttyS1, you need to modify the OpenWRT kernel config accordingly, and rebuild.
For White Russian, the file to modify is `target/linux/linux-2.4/config/brcm`, while for Kamikaze it is `target/linux/brcm-2.4/config`. In both, you need to change
{{{
...
...
This model uses a version of the CFE bootloader 192k bytes long instead of 256k bytes, so the flash mapping is not compatible with White Russian RC5 - it won't boot into the kernel:
