...
Apparently, there are two versions of this hardware. to tell the difference, look at the bottom of the chasis, at the sticker. the sticker on the V1 has no mention of version number on it.

= Default Firmware =

The firmware from the manufacturer runs linux. the following is the output of firmware version 3.0.2 on a version 1 router:
...
Linux version 2.4.18_mvl30-ixdp425 (wg302@Perphect) (gcc version 3.2.1 20020930 (MontaVista)) #1 Sun Mar 27 11:31:09 HKT 2005
CPU: Intel XScale-IXP4xx/IXC1100 revision 1
Machine: Intel IXDP425 Development Platform
Warning: bad configuration page, trying to continue
bootmem_init: rdisk setup
rdisk  start c0039100, end 0019162e
...
Memory: 29140KB available (1053K code, 211K data, 196K init)
...
==============================================
  Driver Build Time: 11:47:08 Mar 27 2005
==============================================
Allocate ZComDmaPool @c15fb720, UnitCount: 640, UnitSize:128 Total Size: 81920
         Host Address:c324a000, Dma Address: 01140000
Allocate ZComDmaPool @c15fb760, UnitCount: 3328, UnitSize:256 Total Size: 851968
         Host Address:c325f000, Dma Address: 01000000
Read MacAddress By ZComWlan_IOControl()
MacAddress: 00:15:6d:53:2c:c1
device ixp0 entered promiscuous mode
device sap0 entered promiscuous mode
}}}
...
{{{
RedBoot> fis create -f 0x50780000 microcode
}}}
...
ifconfig: SIOCSIFFLAGS: Input/output error
 ...
 . Reason: unclear. Maybe some firmware missing? /lib/firmware/ is empty Worksround: After Boot, enter
{{{
...
eth0: link down
 ...
 . Then, I can configure the interface. Still, there ist no link on eth0. eth1 is also not usable after loadinf the firmware, same error message as above.
...
= Second Serial =
 * this unit has a second serial port, which is a 3.3V port. as in, you will have to raise the voltage to 12V levels to hook it to a PC. it is the ten pin un-soldered header toward the front of the PCB. (not the 20 pin in front of that).
 * its pinout is:
...
its worth noting, if you're thinking of using this to de-brick another AP, that you need to cross the CTS and RX lines, connecting them to RX on the receiving AP's end. don't ask me why.
