...
You will start running out of CPU when forwarding packets at about 10-12Mbps of downstream traffic. ADSL2+ connections can do more than this (e.g. I get 15-17Mbps in theory) so that's a bit annoying. It's somewhat hard to narrow down the culprit as OProfile does not support the MIPS 4K CPUs.

The cpmac ethernet driver on its own can manage around 21Mbps of traffic being generated by the local TCP stack (so presumably more than that if the traffic is just being forwarded). There is some room for optimization of the transmit path - currently it generates a completion interrupt for every packet sent, as there is only ever one packet queued for transmission at a time. Adding a transmit ring and moving to a NAPI model on the tx path may help. In comparison, the rx path (which uses a receive ring and NAPI) can handle 40Mbps being received by local TCP with far fewer interrupts.

The ADSL driver is a horrible tangle of HAL layers and OEM code. I wouldn't be too surprised if it was sucking more CPU than it needed to, but it's hard to single out any one bit of that code for cleanup.
