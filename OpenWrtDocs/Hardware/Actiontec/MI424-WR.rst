...
Installation of !RedBoot can be accomplished with the [https://dev.openwrt.org/browser/trunk/scripts/jungo-image.py?format=txt jungo-image.py] script. It requires that a tftp server that can serve the {{{redboot.bin}}}. The script uses the telnet interface into the router to accomplish it's task. Depending on the version of the firmware, it may have to be manually enabled in the advanced tab under local adminstration. The script will first make a backup of the current flash image; this procedure takes about 4 minutes. The actual writing of redboot requires the {{{-w}}} flag. Use {{{-h}}} to get help on all the options. If there's some failure, the only recourse is to install a JTAG header and restore the firmware via JTAG.
...
!RedBoot can be built from the Intel !RedBoot sources found in the [http://www.intel.com/design/network/products/npfamily/download_ixp400.htm Intel IXP400 Software] site. You'll need the !RedBoot source code as well as the !RedBoot NPE microcode. This attachment:mi424wr.epk adds support for the MI424-WR. The procedure for building !RedBoot is as follows:
 ...
 1. {{{cd ${ECOS_REPOSITORY}; ./ecosadmin.tcl add mi424wr.epk}}}
 1. {{{cd <build_dir>; ecosconfig new mi424wr_npe redboot}}}
 1. {{{ecosconfig add memalloc io fileio error linux_compat kernel crc zlib}}}
 1. {{{ecosconfig import ${ECOS_REPOSITORY}/hal/arm/xscale/mi424wr/current/misc/redboot_ROM.ecm}}}
 1. {{{ecosconfig tree}}}
 1. {{{make}}}
...
The openwrt images were built with the following configuration files: attachment:openwrt.config and attachment:kernel.config.
Here's a boot log generated by dmesg:

{{{
BusyBox v1.11.3 (2008-11-28 18:31:24 EST) built-in shell (ash)
Enter 'help' for a list of built-in commands.
 ...
 ---------------------------------------------------
root@OpenWrt:/# dmesg
Linux version 2.6.27.6 (jose@chessmaster) (gcc version 4.2.4) #4 Sun Nov 30 22:06:09 EST 2008
CPU: XScale-IXP42x Family [690541c1] revision 1 (ARMv5TE), cr=000039ff
Machine: Actiontec MI424WR
Memory policy: ECC disabled, Data cache writeback
On node 0 totalpages: 8192
free_area_init_node: node 0, pgdat c01d3e78, node_mem_map c01e8000
...
Memory: 30532KB available (1708K code, 128K data, 76K init)
...
PCI: 0000:00:0d.0 reg 10 32bit mmio: [0, 3ffff]
PCI: 0000:00:0d.1 reg 10 32bit mmio: [0, fff]
PCI: 0000:00:0e.0 reg 10 32bit mmio: [ffffe000, ffffffff]
PCI: 0000:00:0f.0 reg 10 32bit mmio: [0, 3ffff]
PCI: 0000:00:0f.1 reg 10 32bit mmio: [0, fff]
...
mtd: partition "rootfs_data" created automatically, ofs=440000, len=3A0000 
