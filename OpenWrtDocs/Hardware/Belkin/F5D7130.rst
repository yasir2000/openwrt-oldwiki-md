...
- I have a version 1000 and it seems to upgrade to [:OpenWrtDocs/Hardware/Belkin/F5D7230:F5D7230] just fine, but when i upgrade to OpenWRT using TFTP it's still the [:OpenWrtDocs/Hardware/Belkin/F5D7230:F5D7230] firmware after reboot. It shows no errors in uploading the firmware. Uploading the firmware through the normal webinterface tells me that the firmware is incompatible and therefore denies it. Looking into it, it seems that i need to extract something from the original firmware to compile the OpenWRT firmware. Little help, please? Particularly, how do i create the belkin-kernel.bin and belkin-cramfs.bin files? Do i dd out the first 1B (27) bytes from the [:OpenWrtDocs/Hardware/Belkin/F5D7230:F5D7230] firmware? Which of the files would that make and what about the other file? Am i getting it entirely wrong here?
